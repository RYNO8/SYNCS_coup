...
VERSIONS = [
    ...
    "09-16 ????? endgame is when 1 opponent",
    "09-17 00:52 original endgame",
    "09-17 01:10 realised steal is risky, dont challenge as hard",
    "09-17 01:20 dont exchange on first turn if {contessa, assassin}",
    "09-17 01:48 reverted, hopefully >0.7 win",
    "09-17 01:59 kill player with least cards",
    "09-17 03:15 more likely to challenge in endgame",
    "09-17 03:38 foreign aid instead of income in endgame",
    "09-17 03:49 assassin immediately when 3",
    "09-17 04:24 dont need likely predictor for coup",
    "09-17 04:46 testing simulation"
]
VERSION = VERSIONS[-1]
    ...
    while prev_turn >= 0:
        ...
        if prev_action.player == game_info.player_id and prev_action.action == action and prev_action.target == target_id:
            if prev_action.successful:
                ...
                return -1
        elif (target_id is None or prev_action.player == target_id) and prev_action.action == PrimaryAction.Exchange and prev_action.successful:
            return 0
...
def simulate(balances, players_cards_num):
    print(balances, players_cards_num)
    for turn in range(1, 5):
        current_player = (game_info.player_id + turn) % 5
        if balances[current_player] >= 7:
            opponents = []
            for i in range(1, 5):
                opponent = (current_player + i) % 5
                if players_cards_num[opponent] > 0:
                    opponents.append([balances[opponent], opponent])
            best_balance, target = max(opponents)
            assert players_cards_num[target] > 0
            balances[current_player] -= 7
            players_cards_num[target] -= 1
        print(current_player)
        print(balances, players_cards_num)
    return balances, players_cards_num

def opponent_to_kill(action):
    ...
    elif action == PrimaryAction.Coup:
        valid_opponents = get_alive_opponents()
    else:
        raise Exception("invalid kill action")

    if not valid_opponents:
        valid_opponents = get_alive_opponents()
    
    for player_id in valid_opponents:
        print(f"P{player_id} has ${game_info.balances[player_id]} and {game_info.players_cards_num[player_id]} cards.")
    
    # best_
    # for target in valid_opponents:
    #     balances = game_info.balances[:]
    #     players_cards_num = game_info.players_cards_num[:]
    #     if action == PrimaryAction.Assassinate:
    #         balances[game_info.player_id] -= 3
    #     elif action == PrimaryAction.Coup:
    #         balances[game_info.player_id] -= 7
    #     else:
    #         raise Exception("invalid kill action")
    #     assert players_cards_num[target] > 0
    #     players_cards_num[target] -= 1
    #     print(".")
    #     print(balances, players_cards_num)
    #     balances, players_cards_num = simulate(balances, players_cards_num)
        

    choices = [
        [(-game_info.players_cards_num[player_id], game_info.balances[player_id]), player_id]
        for player_id in valid_opponents
    ]
...
def primary_action_handler():
    ...
    # FIRSTLY TRY TO KILL
    # NOTE: using 4 not 3 because delaying is bigbrain
    #if myBalance >= 4 and hasAssassin():
    if myBalance >= 3 and hasAssassin():
        bot_battle.play_primary_action(PrimaryAction.Assassinate, opponent_to_kill(PrimaryAction.Assassinate))
    elif myBalance >= 7:
        bot_battle.play_primary_action(PrimaryAction.Coup, opponent_to_kill(PrimaryAction.Coup))
...
def challenge_action_handler():
    ...
    print(".")
    print(".")
    print(game_info.revealed_cards)
    ...
    global lying
    if game_info.current_primary_player != game_info.player_id or lying:
        bot_battle.play_challenge_action(ChallengeAction.NoChallenge)

    # we played assasssin, they are trying to counter it
    elif previous_action.action == CounterAction.BlockAssassination:
        print("Challenge", "--", "surely youre faking a contessa")
        bot_battle.play_challenge_action(ChallengeAction.Challenge)

    elif previous_action.action == CounterAction.BlockStealingAsCaptain and game_info.revealed_cards[Character.Captain] == 3:
        bot_battle.play_challenge_action(ChallengeAction.Challenge)
    elif previous_action.action == CounterAction.BlockStealingAsAmbassador and game_info.revealed_cards[Character.Ambassador] == 3:
        bot_battle.play_challenge_action(ChallengeAction.Challenge)
    elif previous_action.action == CounterAction.BlockForeignAid and game_info.revealed_cards[Character.Duke] == 3:
        bot_battle.play_challenge_action(ChallengeAction.Challenge)

    elif (
        previous_action.action in [CounterAction.BlockStealingAsCaptain, CounterAction.BlockStealingAsAmbassador, CounterAction.BlockForeignAid] and
        isEndgame()
    ):
        bot_battle.play_challenge_action(ChallengeAction.Challenge)
    else:
...
while True:
    ...
    if just_started:
        ...
        print(f"My player id is P{game_info.player_id}")
        just_started = False
